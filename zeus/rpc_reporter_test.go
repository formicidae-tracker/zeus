...
type olympusStub struct {
	...
	received chan *olympuspb.ZoneUpStream
}

func (o *olympusStub) Zone(s olympuspb.Olympus_ZoneServer) error {
	...
	for {
		...
		s.Send(&olympuspb.ZoneDownStream{})
		o.received <- m
	}
}
...
func (s *RPCClimateReporterSuite) SetUpTest(c *C) {
	...
	s.olympus = &olympusStub{received: make(chan *olympuspb.ZoneUpStream, 10)}
